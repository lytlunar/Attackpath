...
import { calculateAttackPathPriority, type AttackPathPriorityResult } from "../../../lib/priorityScore";

const MAX_BATCH_SIZE = 100;

export type SyntheticPriorityResponse = {
  synthetic: true;
  acceptedEventCount: number;
  rejectedEventCount: number;
  detectionCount: number;
  score: AttackPathPriorityResult;
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const score = calculateAttackPathPriority(detections, graph);

    const responseData: SyntheticPriorityResponse = {
      synthetic: true,
